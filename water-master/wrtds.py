...
sn = 1e-5
log_every = 5
save_path = "save_wrtds"

train_dfs = []
test_dfs = {}

with open("../climate_washed/train_test_split.txt", "r") as f:
    for line in f.readlines():
        idx, mode = line.split(" ")
        mode = mode.strip()
        
        df = pd.read_csv(os.path.join("../climate_washed/", f"{idx}.csv"))
        if mode == 'train':
            train_dfs.append(df)
        else:
            test_dfs[idx] = df
...
for id, test_df in test_dfs.items():
    ...
    if os.path.exists(os.path.join(save_path, f"{id}.pkl")):
        ...
        nse_list.append(kge(state['pred'], test_df[varY].values).numpy())
    ...
    nse_list.append(kge(yOut, y_test).numpy())
    r2_list.append(r_squared(yOut, y_test).numpy())
    pbias_list.append(pbias(yOut, y_test).numpy())

print("Final NSE: ", np.nanmedian(nse_list, axis=0))
print("Final R2: ", np.nanmedian(r2_list, axis=0))
print("Final PBIAS: ", np.nanmedian(pbias_list, axis=0))

with open("wrtds_result.txt", "w") as f:
    f.write(f"NSE: {np.nanmedian(nse_list, axis=0)}\n")
    f.write(f"R2: {np.nanmedian(r2_list, axis=0)}\n")
    f.write(f"PBIAS: {np.nanmedian(pbias_list, axis=0)}\n")