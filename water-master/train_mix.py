...
from trainer import TrainerOne
from utils import prepare_device
from predictone import predict_whole_dataset
...
def main(config: ConfigParser):
    ...
    if config['dataset'] == 'ClimateDataset':
        ...
        train_data = data_class("../climate_washed", config['normalize'], split="train", x_feature=config['x_feature'], y_feature=config['y_feature'],
                                exclude=config['exclude'], location_static=config['location_static'])
        stats = train_data.get_stats()
        val_data = data_class("../climate_washed", config['normalize'], split="test", stats=stats,
                              x_feature=config['x_feature'], y_feature=config['y_feature'], exclude=config['exclude'], location_static=config['location_static'])
    ...
    model = module_model.LSTMOne(**config['arch'])
    logger.info(model)
    ...
    if config['y_feature']:
        ...
        num_class = 20
    trainer = TrainerOne(model, criterion, metrics, optimizer,
                      config=config,
                      device=device,
