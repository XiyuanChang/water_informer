...
import lstm_dataset_pretrain as dataset
...
def _prepare_informer_inputs(x, label_len=96):
    ...
    # pred part: repeat last observed x for pred_len slots (no future x available)
    last_x = x_data[:, -1:, :]                                         # [B, 1, F]
    x_dec_pred = last_x.repeat(1, pred_len, 1)                          # [B, pred_len, F]
    # Concatenate label and prediction parts
    x_dec = torch.cat([x_dec_label, x_dec_pred], dim=1)  # [B, label_len + pred_len, features]
...
def predict_whole_dataset(model, state_dicts, stats, config, valid_threshold=51, normalize='MeanStdNormalize',
                          split='split_datesA.txt', root="../climate_washed", output_dir="output",
    ...
    root_dir = config['root_dir'] if config['root_dir'] is not None else "../climate_washed"
    ...
    # data = valset("../climate_washed", config['normalize'], split="test", stats=stats, testNum=20)
    ...
    for staids, date, x, y in tqdm(loader):
        ...
        target = y[:, -1, :].cpu()
        #target = y[:,1,:].cpu()  # pred_len=2 时，取 t+1 作为目标
        x = x[:, -1, :x_feat].cpu()
    ...
    for staid in targets.keys():
        ...
        allowed_dates = new_df['Date'].tolist()
...
if __name__ == "__main__":
    ...
    # path = "unified_ablation_informer_cpu10/baseline_dropout_0.1_665/models/informer/0929_144935/checkpoint-epoch70.pth"
    # config = "unified_ablation_informer_cpu10/baseline_dropout_0.1_665/models/informer/0929_144935/config.json"
    path = "unified_ablation_informer_pretrain/baseline_dropout_0.1_665/models/informer/1006_043917/best_target_state_dict.pth"
    config = "unified_ablation_informer_pretrain/baseline_dropout_0.1_665/models/informer/1006_043917/config.json"
    ...
    def strip_module(sd: dict):
        # 兼容 DP/DDP 保存的 'module.' 前缀
        return {k.replace("module.", ""): v for k, v in sd.items()}

    # 统一整理成一个 list[ state_dict ]，后面逐个 load 即可
    model_states = []
    if isinstance(raw, dict) and "state_dict" in raw and isinstance(raw["state_dict"], dict):
        # 典型：{'state_dict': {...}, ...}
        model_states = [strip_module(raw["state_dict"])]
    elif isinstance(raw, dict) and all(isinstance(v, torch.Tensor) for v in raw.values()):
        # 直接就是 state_dict
        model_states = [strip_module(raw)]
    elif isinstance(raw, (list, tuple)):
        # 多份 state_dict 的列表
        model_states = [strip_module(sd) for sd in raw]
    elif isinstance(raw, dict):
        # 多目标/多模型的字典（例如 {'0': sd0, '1': sd1, ...} 或 {'target_x': sd, ...}）
        model_states = [strip_module(sd) for sd in raw.values()]
    else:
