...
with open("../climate_washed/split_datesB.txt", "r") as f:
...
for id, test_df in test_dfs.items():
    ...
    # skip the rows with less than 30 valid values
    ...
    # kge_dict[id] = kge(yOut, y_test).numpy()
    # r2_dict[id] = r_squared(yOut, y_test).numpy()
    # pbias_dict[id] = pbias(yOut, y_test).numpy()
...
metric_df.to_csv("wrtdsB_metric.csv", index=False, na_rep='')
...
# with open("wrtdsB_result.json", "w") as f:
#     d = {
#         "NSE": nse_list,
#         "R2": r2_list,
#         "PBIAS": pbias_list
...
# with open("wrtdsB_result.txt", "w") as f:
