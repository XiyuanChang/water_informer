...
class InformerTrainer(LSTMTrainer):
    ...
    def _prepare_informer_inputs(self, x, y):
        ...
        y_target = y[:, -self.pred_len:, :]  # [B, 1, targets] - only the last timestep
        ...
        #加把当天的(t)特征复制 pred_len 次当作预测占位（不需要 next-day 的 x）
        x_last = x_data[:, -1:, :]  
        x_dec_pred = x_last.repeat(1, self.pred_len, 1)   # [B, pred_len, in_dim]
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (x, y) in enumerate(self.data_loader):
            ...
            output_sel = output[:, :, :20]
            y_target_sel = y_target[:, :, :20]
            # Use the same loss function as LSTM - output is [B, 1, targets]
            _, loss_ = self.criterion(output_sel, y_target_sel, threshold=threshold)
            ...
            if epoch % self.log_epoch == 0:
                outputs.append(output_sel[:, -1, :].detach().cpu())  # Last prediction timestep
                target = y_target_sel[:, -1, :].cpu()  # Ground truth for last timestep
        ...
        if self.valid_data:
            ...
            for i in range(len(self.valid_data)):
                ...
                for batch_idx in range(0, x.shape[0], 4096):
                    ...
                    pred_sel = pred[:, :, :20]
                    y_target_sel = y_target[:, :, :20]
                    loss_per_sample, per_target_loss = self.criterion(pred_sel, y_target_sel)
                    
                    pred_sel = pred_sel.detach().cpu()
                    pred_y = pred_sel[:, -1, :]  # Last prediction timestep
                    y_target_sel = y_target_sel[:, -1, :].cpu()  # Ground truth for last timestep
                    target = torch.where(y_target_sel == -1, torch.tensor(float('nan')), target)
