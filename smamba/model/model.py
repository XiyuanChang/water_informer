...
class InformerWrapper(nn.Module):
    def __init__(self, in_dim, out_dim, seq_len=96, label_len=48, pred_len=1, 
        ...
        # For simplicity, assume time features are embedded separately
        # In practice, you might need to adapt this based on your data structure
        enc_in = in_dim
        dec_in = in_dim  
        c_out = out_dim
    ...
    def forward(self, x):
        ...
        x = torch.where(x == -1, torch.zeros_like(x), x)  # Replace -1 with 0
        
        # x shape: [batch_size, seq_len, features]
        B, L, D = x.shape
        
        # For time series forecasting, create decoder input
        zeros = torch.zeros([B, self.pred_len, D]).to(x.device)
        dec_inp = torch.cat([x[:, -self.label_len:, :], zeros], dim=1)
        
        # Create simple time marks (you might want to use actual time features)
        x_mark_enc = torch.zeros([B, L, 3]).to(x.device)  # dummy time features
        x_mark_dec = torch.zeros([B, self.label_len + self.pred_len, 3]).to(x.device)
        
        output = self.model(x, x_mark_enc, dec_inp, x_mark_dec)
        
        # Return in format compatible with LSTM output
        # Expand output to match sequence length if needed
        # Return in format compatible with LSTM output
        # Expand output to match sequence length if needed
        full_output = torch.zeros([B, L, output.shape[-1]]).to(x.device)
        full_output[:, -self.pred_len:, :] = output
        
        return full_output

    # --- SHAP helper ---
    def predict_function_for_shap(self, X):
        """Prediction function compatible with shap.Explainer.

        Accepts either a 2D array of shape [n_samples, seq_len*in_dim]
        or a 3D array [n_samples, seq_len, in_dim], and returns predictions
        for the last time step with shape [n_samples] if out_dim==1 else
        [n_samples, out_dim].
        """
        import numpy as np
        import pandas as pd
        ...
        X = np.asarray(X)
        if X.ndim == 2 and X.shape[1] == self.seq_len * self.in_dim:
            X = X.reshape(-1, self.seq_len, self.in_dim)
        elif X.ndim == 3 and X.shape[1:] == (self.seq_len, self.in_dim):
            pass
        else:
            raise ValueError(
                f"Expected input of shape [n, {self.seq_len*self.in_dim}] or [n, {self.seq_len}, {self.in_dim}], got {X.shape}")

        with torch.no_grad():
            xt = torch.from_numpy(X).float().to(device)
            yt = self.forward(xt)  # [B, L, out_dim]
            yt = yt[:, -1, :]      # last step -> [B, out_dim]
            if yt.shape[-1] == 1:
                return yt.squeeze(-1).cpu().numpy()
            return yt.cpu().numpy()