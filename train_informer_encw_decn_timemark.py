from model.informer_xenc365_dec96 import LSTM, InformerWrapper
from parse_config import ConfigParser
import argparse
import collections
import dataset_enc_dec as dataset
import model.lstm_loss as module_loss
from torch.utils.data import DataLoader, WeightedRandomSampler
from utils import MetricTracker
from trainer import LSTMTrainer
from trainer.informer_enc365_dec96 import InformerTrainer
import model.metric as module_metric
from utils import prepare_device
import torch
import numpy as np
from predict_enc365_dec_96 import predict_whole_dataset
import warnings
...
def main(config: ConfigParser):
    ...
    assert config['dataset'] in ['ClimateDatasetV2A', 'ClimateDatasetV2B', 'ClimateDatasetV2C', 'ClimateDatasetV2D','ClimateDatasetV2C_dual'], "Dataset still not supported"
    ...
    #valid_data_loader = DataLoader(val_data, batch_size=1024, num_workers=10, pin_memory=True, shuffle=False)
    ...
    last_ckpt_path = str(trainer.checkpoint_dir / "last_state_dict.pth")
    ...
    state_dict = torch.load(last_ckpt_path, map_location="cpu")
    ...
    split_dict = {
        ...
        "ClimateDatasetV2C_dual": "split_datesC.txt",
...
if __name__ == '__main__':
    ...
    args.add_argument('-c', '--config', default='config_informer.json', type=str,
                      help='config file path (default: config_informer.json)')
